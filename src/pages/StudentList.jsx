...
function StudentList() {
  ...
  const visibleStudents = useMemo(() => {
    ...
    return filtered;
  }, [students, searchTerm, currentStudentIndex]);
  ...
  return (
    <main style={{ padding: "20px", backgroundColor: '#0d1117', color: '#c9d1d9', minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #30363d', borderRadius: '5px' }}>
        ...
        <p>
          <strong>Students Found:</strong> <strong>{studentCount}</strong>
          {studentCount > 0 && (
            ...
            {filterCriteria.backgrounds && currentStudent.backgrounds && (
              <div style={{
                ...
                {currentStudent.backgrounds.academic && (
                  <p style={{ margin: 0 }}>
                    <strong>Academic:</strong> {currentStudent.backgrounds.academic}
                  </p>
                )}
                {currentStudent.backgrounds.professional && (
                  <p style={{ margin: 0 }}>
                    <strong>Professional:</strong> {currentStudent.backgrounds.professional}
                  </p>
                )}
              </div>
            )}
                ...
                <ul style={{ listStyleType: 'disc', paddingLeft: '20px', margin: 0 }}>
                  {currentStudent.courses.map((course, index) => (
                    <li key={index} style={{ marginBottom: '5px' }}>
                      <strong>{course.code || `${course.dept} ${course.num}`}</strong>: {course.name}
                      {course.reason && (
                        <span style={{
                          display: 'block',
                          fontSize: '0.8em',
            ...
            {filterCriteria.quote && currentStudent.quote && (currentStudent.quote.text || currentStudent.quote.author) && (
              <blockquote style={{
                ...
                {currentStudent.quote.author && (
                  <span style={{ display: 'block', fontSize: '0.8em', marginTop: '5px' }}>
                    — {currentStudent.quote.author}
                  </span>
                )}
              </blockquote>
